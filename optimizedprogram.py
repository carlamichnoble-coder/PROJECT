...
class DoctorDialog(QDialog):
    ...
    def _save(self):
        ...
        if doctors_bst.search(name) is not None:
            QMessageBox.warning(
                self, "Duplicate Doctor",
                f"A doctor named \"{name}\" already exists.\n"
                "Please use a different name or edit the existing record."
            )
            return
...
class PatientDialog(QDialog):
    ...
    def _save(self):
        name    = self.name_edit.text().strip()
        if any(p["name"].lower() == name.lower() for p in patients_map.values()):
            QMessageBox.warning(
                self, "Duplicate Patient",
                f"A patient named \"{name}\" already exists.\n"
                "Please verify the record or use a different name."
            )
            return
...
class DashboardPanel(QWidget):
    ...
    # Today's Appointments — teal light bg, dark teal text
        card1, label1 = self._make_stat_card_with_label("  Today's Appointments", "#E1F5EE", "#085041")
        self.stat_cards["today"] = label1
        stats_layout.addWidget(card1, 0, 0)

    # Total Patients — blue light bg, dark blue text
        card2, label2 = self._make_stat_card_with_label("  Total Patients", "#E6F1FB", "#0C447C")
        self.stat_cards["patients"] = label2
        stats_layout.addWidget(card2, 0, 1)

    # Doctors On Duty — teal dark bg, light text
        card3, label3 = self._make_stat_card_with_label("  Doctors On Duty", "#0F6E56", "#E1F5EE")
        self.stat_cards["doctors"] = label3
        stats_layout.addWidget(card3, 0, 2)

    # Pending Records — amber light bg, dark amber text
        card4, label4 = self._make_stat_card_with_label("  Pending Records", "#FAEEDA", "#854F0B")
    ...
    def _make_stat_card_with_label(self, label: str, bg_color: str, text_color: str) -> tuple:
        ...
        lbl = QLabel(label)
        lbl.setStyleSheet(f"color: {text_color}; font-size: 13px; font-weight: 600; background: transparent; border: none;")
        lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)

        val = QLabel("0")
        val.setStyleSheet(f"color: {text_color}; font-size: 36px; font-weight: 800; background: transparent; border: none;")
    ...
    def _handle_load_data(self):
        try:
            if load_data():
                ...
                self.dashboard_panel._refresh_stats()
