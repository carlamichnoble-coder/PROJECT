class _BSTNode:
    ...
    def __init__(self, key: str, data: dict):
        self.key   = key      # doctor name (lowercase, for comparison)
...
class DoctorBST:
    ...
    def _insert(self, node, key, data):
        ...
        if key < node.key:
            ...
            node.data = data   # update if same name
        return node

    def search(self, name: str) -> dict | None:
        """O(log n) average — find doctor by exact name."""
    ...
    def _prefix_scan(self, node, prefix, results):
        ...
        # Prune left subtree only if prefix is definitely greater
        if prefix <= node.key:
    ...
    def _delete(self, node, key):
        ...
        if key < node.key:
            ...
            if node.right is None:
                return node.left
            # Two children: replace with in-order successor (leftmost of right)
    ...
    def inorder(self) -> list:
        """Return all doctors sorted A→Z by name. O(n)."""
        result = []
...
def greedy_available_slots(appt_map: DateAppointmentMap,
    ...
    if date_str in appt_map:
        ...
        for _, time_val, appt in heap:
            if appt.get("doctor", "") == doc_label:
                booked.append(time_str_to_minutes(time_val))

    for i in range(1, len(booked)):
        key = booked[i]
        j   = i - 1
        while j >= 0 and booked[j] > key:
            booked[j + 1] = booked[j]
            j -= 1
        booked[j + 1] = key
    ...
    while candidate + slot_duration <= end_mins:
        conflict = any(abs(candidate - b) < slot_duration for b in booked)
        if not conflict:
